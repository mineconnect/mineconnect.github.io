...
export default function UserManagement({ userProfile }: UserManagementProps) {
  ...
  const [] = useState<UserProfile | null>(null)
  ...
  // Filtro de perfiles (Admin ve todos; no oculta usuarios creados por Admin)
      ...
      {isFormOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4 bg-slate-800 p-4 rounded-lg">
          <form className="space-y-4">
            <h2 className="text-lg font-semibold text-white">{/* agregar título dinámico si se desea */}Nuevo Conductor</h2>
            ...
            <input type="password" placeholder="Contraseña" value={password} onChange={e => setPassword(e.target.value)} required className="w-full p-2 rounded bg-slate-700 text-white border border-slate-600" />
        ...
        <table className="min-w-full divide-y divide-slate-700">
          <thead className="bg-slate-700/50">
            <tr>
              ...
              { /* Empresa visible solo para Admin */ isSuperAdmin && (
